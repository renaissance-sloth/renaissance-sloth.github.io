---
title: "Iteration"
excerpt: Iteration tags repeatedly run blocks of code.
tags: liquid tags iteration
header:
...
## for
Repeatedly executes a block of code. For a full list of attributes available within a `for` loop, refer to the [`forloop` object]({{page.url | append:"#forloop-object"}}).
{% raw %}
  ```javascript
//input
...
## forloop (object)
Information about a parent [`for` loop]({{page.url | append:"#for"}}).
{% raw %}
  ```javascript
{
  "first": true,
...
`cycle` must be used within a [for]({{page.url | append:"#for"}}) loop block.
...
## tablerowloop (object)
Information about a parent [`tablerow` loop]({{page.url | append:"#tablerow"}}).
{% raw %}
```html
{
  "col": 1,
