...
이때, $n_{i}$는 이온화된 원자의 밀도($\text{cm}^{3}$당 개수), $n_{n}$은 중성 원자의 밀도($\text{cm}^{3}$당 개수), $T$는 온도($\text{K}$), $K$는 볼츠만 상수(약 $1.380\times10^{−23} \text{J/K}$), $U_{i}$는 이온화 에너지($\text{J}$)다.

대기에 대해 계산해보자면, $n_{n}=3\times10^{19}\text{cm}^{-3}$, $T=300^{\circ}\text{K}$, $U_{i}=14.5\text{eV}$, $\frac{n_{i}}{n_{n}＋n_{i}} \approx \frac{n_{i}}{n_{n}}$로 식에 의해 예상되는 값은 $\frac{n_{i}}{n_{n}} \approx 10^{-122}$이다.
...
밀도 $n$(단위 부피당 입자 수, $\text{cm}^{-3}$)은 모든 속도의 입자 수와 같기 때문에 $n=\int_{-\infty}^{\infty} {f(u)du}$로 주어지고, $A=n\left(\frac{m}{2\pi KT}\right)^{1/2}$으로 계산할 수 있다. 

---
<details>
<summary>$A$를 계산하는 방법</summary>
...
$KT$ $=1 \text{eV}$ $= 1.6\times 10^{-19} \text{J}$일 때, $T$ $= \frac{1.6\times10^{-19}}{1.38\times10^{-23} }$ $\approx 11600 ^{\circ}\text{K}$이다.

핵융합이 일어나기 위해서는 $10\text{keV}$ 정도의 온도가 필요하다. 밀도는 $10^{14}\text{cm}^{-3}$보다 낮아야 한다.
...
고온이라고 해서 꼭 많은 열을 뜻하지 않는다. 형광등의 경우 내부의 전자 온도가 약 $20,000^{\circ}\text{K}$이지만, 전자 밀도가 대기압보다 훨씬 낮기 때문에 열용량은 크지 않다. 따라서 전달되는 총 열량은 고온에도 불구하고 적은 것이다.

실험실에서 사용하는 플라즈마는 보통 $1,000,000^{\circ}\text{K} (100 \text{eV})$정도로 뜨겁지만, 밀도가 $\text{cm}^{3}$당 $10^{12}-10^{13}$정도에 불과해 플라즈마를 담은 벽이 뜨거워질 걱정은 덜어도 된다.

## Debye 차폐
플라즈마는 기본적으로 전위를 차폐한다.

배터리로 연결되어 대전된 두개의 공을 플라즈마 내부에 넣어보자.

<!-- \TODO(3/10) 이미지 넣기 -->

공은 반대 전하를 띈 입자를 끌어당겨 이온 구름이 공을 둘러싸게 된다.
...
## 플라즈마 물리의 활용
<!-- \TODO(3/12) 설명 추가  -->
플라즈마는 밀도와 온도로 특정할 수 있다. 플라즈마는 매우 광범위하게 활용된다. 밀도는 $1-10^{28}\text{cm}^{-3}$, 온도는 $0.1-10^{6}\text{eV}$ 정도로 다양하다.

물은 공기보다 $10^{3}$배 밀도가 높다. 백색왜성은 공기보다 $10^{8}$배 밀도가 높다. 중성자별조차도 공기보다 밀도가 $10^{18}$배 높다는 것을 감안하였을 때, 최대 $10^{28}$배 차이나는 플라즈마는 정말 다양하다고 볼 수 있다.

이러한 다양한 밀도 범위에서도 플라즈마는 고전역학으로 모두 설명할 수 있다.
